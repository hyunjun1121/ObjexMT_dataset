"""
Comprehensive statistical analysis for OBJEX dataset - PARALLEL VERSION
Optimized for 128-core server with multiprocessing
"""
...
def bootstrap_iteration(args):
    ...
    correct1_arr, correct2_arr, seed_offset = args
    np.random.seed(seed_offset)
    n = len(correct1_arr)
    idx = np.random.choice(n, n, replace=True)
    return correct1_arr[idx].mean() - correct2_arr[idx].mean()

def paired_bootstrap_test_parallel(data1, data2, n_bootstrap=10000, base_seed=42):
    """
    Parallel version of paired bootstrap test
    """
    # Ensure same samples
    merged = pd.merge(
    ...
    args_list = [(correct1_arr, correct2_arr, base_seed + i) for i in range(n_bootstrap)]

    # Parallel bootstrap
    with Pool(N_JOBS) as pool:
        bootstrap_diffs = pool.map(bootstrap_iteration, args_list)
...
def process_single_comparison(args):
    """Process a single model pair comparison"""
    model1, model2, data1, data2 = args

    # Paired bootstrap
    bootstrap_result = paired_bootstrap_test_parallel(data1, data2)
...
def compute_all_pairwise_tests_parallel(data):
    """
    Compute all pairwise comparisons in parallel
    """
    ...
    print(f"Processing {len(comparison_args)} pairwise comparisons in parallel...")
...
def process_dataset_comparison(args):
    ...
    if len(data1_ds) > 0 and len(data2_ds) > 0:
        ...
        bootstrap_result = paired_bootstrap_test_parallel(data1_ds, data2_ds, n_bootstrap=5000)
...
def compute_dataset_wise_tests_parallel(data):
    """
    Compute significance tests per dataset in parallel
    """
    ...
    print(f"Processing {len(comparison_args)} dataset-wise comparisons in parallel...")
...
def main():
    ...
    print(f"Starting parallel statistical analysis with {N_JOBS} processes...")
    print("="*60)

    print("Loading data...")
    data, original_results = load_complete_data()

    print("\nComputing comprehensive pairwise tests (parallel)...")
    pairwise_start = time.time()
    pairwise_results = compute_all_pairwise_tests_parallel(data)
    print(f"  Completed in {time.time() - pairwise_start:.1f} seconds")

    print("\nComputing dataset-wise tests (parallel)...")
    dataset_start = time.time()
    dataset_results = compute_dataset_wise_tests_parallel(data)
    ...
    print(f"Speed improvement: ~{128/N_JOBS:.0f}x faster than single-core")
